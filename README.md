...
Для развёртки проекта локально нужно иметь установленный и настроеннй **docker** и **Git**.
...
- Не был прикручен phpStan для проверки ошибок

Остальной описанный функционал был как-то да реализован, а требования были выполнены (в основном).

<h2><a name="postman">Тестирование API через Postman</a></h2>
...
Сначала: http://localhost:8069/sanctum/csrf-cookie
Копируем захэшированное содержание cookie-файла. 
...
В X-XSRF-TOKEN как раз и вставляем скопированный хэш. У вставленного хэша стираем символы с конца до символа ноль.

Потом: http://localhost:8069/api/auth?email=admin@admin.com&password=123, после чего пользователь считается авторизованным
...
Комментарии будут касательно паттерна "Репозиторий".
...
В классе моего собственного производства ``WordsWrapper`` был положен болт на ``SOLID``. Как минимум, буква ``O`` точно не соблюдена, так как в методе ``wrap`` существует ветвление в виде ``if else``, что, например, при добавлении нового языка для паст, потребует модификации класса, а нужно расширение. Исправлено всё это дело не было, потому что мне не хватило времени. Оправдание плохое, согласен, но как есть.
